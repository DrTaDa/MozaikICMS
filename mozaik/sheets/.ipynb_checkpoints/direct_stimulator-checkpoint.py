...
By direct stimulation here we mean an artificial stimulation that
would happen during electrophysiological experiment - such an injection
...
class BackgroundActivityBombardment(DirectStimulator):
    """
    The BackgroundActivityBombardment simulates the poisson disturbated background bombardment of spikes onto a
...
class IntraCorticalMicroStimulation(DirectStimulator):
    ...
    the electrode.
    During stimulation, all selected neurons are connected to Poisson Spike
    Sources. The rate of these spike sources are chosen such that the number of
    spikes generated by the neuron is a log function of the current amplitude.
    ...
    def select_stimulated_cells(self):
        ...
        if "Inh" in self.sheet.name:
            amplitude_scaler = self.parameters.distance_scaler_a
        else:
            amplitude_scaler = 0.5 * self.parameters.distance_scaler_a

        rescale_factor = numpy.sqrt(numpy.clip(
            amplitude_scaler * (self.parameters.amplitude - self.parameters.distance_scaler_b),
            0,
            numpy.inf
        ))

        # This calculates the probability P(activate | distance, current_amplitude)
        scaled_probabilities = self.activation_distribution[1][_idx_distribution] * rescale_factor
    ...
    def plot_probes_and_cells(self):
        """ Debug plot function. For each electrodes, plot the cells activated by the electrode.
        Warning: only plot the active electrodes. If not electrodes are active, no plots will be created."""
        
        cells_positions = self.sheet.pop.positions.T
        cells_positions = [list(self.sheet.vf_2_cs(cp[0], cp[1])) + [cp[2]] for cp in cells_positions]
        cells_ids = self.sheet.pop.all_cells

        for i, cp in enumerate(self.probe.contact_positions):
            if i in self.parameters.probe_active_electrodes:

                fig = plt.figure(figsize=(10, 10))
                ax = fig.add_subplot(projection='3d')

                for cell_id, cp in zip(cells_ids, cells_positions):
                    color = 'gray'
                    if cell_id in self.stimulated_cells and i in self.stimulated_cells[cell_id]:
                        color = "black"
                for j, cp2 in enumerate(self.probe.contact_positions):
                    color = 'blue'
                    if i == j:
                        color = 'red'
                    ax.scatter(cp2[0], cp2[1], cp2[2], c=color)

                ax.set_xlabel('X')
                ax.set_ylabel('Y')
                ax.set_zlabel('Z')

                plt.savefig(f"probes_and_cells_{i}.pdf")
                plt.clf()

                fig = plt.figure(figsize=(10, 10))
                ax = fig.add_subplot(projection='3d')

                for cp in cells_positions:
                    distance = cdist([cp], [self.probe.contact_positions[i]])
                    ax.scatter(cp[0], cp[1], cp[2], c=cm.hot(distance / 2000))
