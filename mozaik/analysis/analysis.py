...
class NeuronToNeuronAnalogSignalCorrelations(Analysis):
    ...
    def perform_analysis(self):
        for sheet in self.datastore.sheets():
            print("Memory NeuronToNeuronAnalogSignalCorrelations ", process.memory_info().rss / 1024 / 1024 / 1024, "Gb")
            # Load up spike trains for the right sheet and the corresponding stimuli, and
            # transform spike trains into psth
            dsv = queries.param_filter_query(self.datastore, sheet_name=sheet, identifier='AnalogSignalList')
            for asl in dsv.get_analysis_result():
                ...
                self.datastore.full_datastore.add_analysis_result(
                    PerNeuronPairValue(corrs,asl.ids,qt.dimensionless, value_name='Correlation coefficient(' +asl.y_axis_name + ')',
                                       sheet_name=sheet,
                                       tags=self.tags,
                                       period=None,
                                       analysis_algorithm=self.__class__.__name__,
...
class Analog_MeanSTDAndFanoFactor(Analysis):
      ...
      def perform_analysis(self):
            ...
            sheets = list(set(self.datastore.sheets()) & set(['V1_Exc_L4','V1_Inh_L4','V1_Exc_L2/3','V1_Inh_L2/3']))
            for sheet in sheets:
                dsv = queries.param_filter_query(self.datastore, sheet_name=sheet)
                segs1, stids = colapse(dsv.get_segments(),dsv.get_stimuli(),parameter_list=['trial'],allow_non_identical_objects=True)
                for segs,st in zip(segs1, stids):
      ...
      def perform_analysis(self):
            ...
            for sheet in sheets:
                ...
                for segs,st in zip(segs1, stids):
                    esyn_ids = segs[0].get_stored_esyn_ids()
                    isyn_ids = segs[0].get_stored_isyn_ids()
                    numpy.testing.assert_array_equal(esyn_ids, isyn_ids)
                    ...
                    # save in datastore
                    self.datastore.full_datastore.add_analysis_result(
                        PerNeuronValue(esyn_var,esyn_ids,segs[0].get_esyn(esyn_ids[0]).units,value_name = 'Var(ECond)',sheet_name=sheet,tags=self.tags,period=None,analysis_algorithm=self.__class__.__name__,stimulus_id=str(st)))        
                    self.datastore.full_datastore.add_analysis_result(
                        PerNeuronValue(isyn_var,isyn_ids,segs[0].get_isyn(isyn_ids[0]).units,value_name = 'Var(ICond)',sheet_name=sheet,tags=self.tags,period=None,analysis_algorithm=self.__class__.__name__,stimulus_id=str(st)))        
                    self.datastore.full_datastore.add_analysis_result(
                        PerNeuronValue(variance_ratio,isyn_ids,segs[0].get_isyn(isyn_ids[0]).units,value_name = 'Var(ECond)/Var(ICond)',sheet_name=sheet,tags=self.tags,period=None,analysis_algorithm=self.__class__.__name__,stimulus_id=str(st)))        
                    self.datastore.full_datastore.add_analysis_result(
                        PerNeuronValue(mean_ratio,isyn_ids,segs[0].get_isyn(isyn_ids[0]).units,value_name = 'Mean(ECond)/Mean(ICond)',sheet_name=sheet,tags=self.tags,period=None,analysis_algorithm=self.__class__.__name__,stimulus_id=str(st)))        
