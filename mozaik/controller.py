...
def prepare_workflow(simulation_name, model_class):
    ...
    # HACK: REMOVE THE EXPERIMENT RELATED PARAMETERS
    _modified_parameters_filtered = {}
    experiment_parameters = {}
    for k, v in modified_parameters.items():
        if "experiment_" in k:
            experiment_parameters[k] = v
        else:
            _modified_parameters_filtered[k] = v
    parameters = load_parameters(parameters_url, _modified_parameters_filtered)
...
def run_workflow(simulation_name, model_class, create_experiments, save_results=True):
    ...
    sim, num_threads, parameters, experiment_parameters = prepare_workflow(simulation_name, model_class)
